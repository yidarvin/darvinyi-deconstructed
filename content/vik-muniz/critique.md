...
1. `05-double-elvis`: the vertical primitive at x0.488 is labelled `PAIRING AXIS`, but the current proof visibly contains one standing Elvis figure, not a pair. This is a wrong-count/structure claim, not a placement margin. Rename the primitive to the visible central figure/portrait axis (and remove the corresponding “preserving the pair” wording in `sources.md`); retain the work title *Double Elvis* and the documented reference to Warhol's *Double Elvis* as historical context.
