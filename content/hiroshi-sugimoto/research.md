# Hiroshi Sugimoto — research note
...
## Visual method in these reproductions

The measured analysis separates the two series cleanly. In the *Seascapes*, the strongest horizontal falls between y0.492 and y0.643, while edge density is exceptionally low (0.000–0.002): the image is organized by a horizon and tonal mass rather than by discrete objects. *Caribbean Sea, Jamaica* ranges from p05 0.220 to p95 0.910; *Baltic Sea, Rügen* stays compressed between 0.094 and 0.216. In the *Theaters*, a luminous screen creates much broader tonal range — p95 0.788 to 0.988 — and more edge information (0.027–0.152), enough for curtains, seats, arches, and screens to become a single ordered architecture. These are measurements of the supplied web reproductions, not claims about the original prints.

## Sources used

- Artist, gallery, museum, and Princeton links documented in [sources.md](sources.md).
- Ingested-file metadata in [manifest.json](manifest.json), treated only as delivery-file metadata where applicable.
